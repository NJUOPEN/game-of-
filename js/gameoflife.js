...
function canSurvive(c22i, c22j, c11i, c11j, c21i, c21j, c31i, c31j, c12i, c12j, c32i, c32j, c13i, c13j, c23i, c23j, c33i, c33j){
...
function generation(){
	...
	for(var i = 1; i != bWidth - 1; ++i){
		for(var j = 1; j != bHeight - 1; ++j){
			nextCell[i][j] = canSurvive(i, j, i - 1, j - 1, i, j - 1, i + 1, j - 1, i - 1, j, i + 1, j, i - 1, j + 1, i, j + 1, i + 1, j + 1);
		}
	}
	//top
	for(var i = 1; i != bWidth - 1; ++i){
		nextCell[i][0] = canSurvive(i, 0, i - 1, bHeight - 1, i, bHeight - 1, i + 1, bHeight - 1, i - 1, 0, i + 1, 0, i - 1, 1, i, 1, i + 1, 1);
	}
	//down
	for(var i = 1; i != bWidth - 1; ++i){
		nextCell[i][bHeight - 1] = canSurvive(i, bHeight - 1, i - 1, bHeight - 2, i, bHeight - 2, i + 1, bHeight - 2, i - 1, bHeight - 1, i + 1, bHeight - 1, i - 1, 0, i, 0, i + 1, 0);
	}
	//left
	for(var j = 1; j != bHeight - 1; ++j){
		nextCell[0][j] = canSurvive(0, j, bWidth - 1, j - 1, 0, j - 1, 1, j - 1, bWidth - 1, j, 1, j, bWidth - 1, j + 1, 0, j + 1, 1, j + 1);
	}
	//right
	for(var j = 1; j != bHeight - 1; ++j){
		nextCell[bWidth - 1][j] = canSurvive(bWidth - 1, j, bWidth - 2, j - 1, bWidth - 1, j - 1, 0, j - 1, bWidth - 2, j, 0, j, bWidth - 2, j + 1, bWidth - 1, j + 1, 0, j + 1);
	}
	//00
	nextCell[0][0] = canSurvive(0, 0, bWidth - 1, bHeight - 1, 0, bHeight - 1, 1, bHeight - 1, bWidth - 1, 0, 1, 0, bWidth - 1, 1, 0, 1, 1, 1);
	//10
	nextCell[bWidth - 1][0] = canSurvive(bWidth - 1, 0, bWidth - 2, bHeight - 1, bWidth - 1, bHeight - 1, 0, bHeight - 1, bWidth - 2, 0, 0, 0, bWidth - 2, 1, bWidth - 1, 1, 0, 1);
	//01
	nextCell[0][bHeight - 1] = canSurvive(0, bHeight - 1, bWidth - 1, bHeight - 2, 0, bHeight - 2, 1, bHeight - 2, bWidth - 1, bHeight - 1, 1, bHeight - 1, bWidth - 1, 0, 0, 0, 1, 0);
	//11
	nextCell[bWidth - 1][bHeight - 1] = canSurvive(bWidth - 1, bHeight - 1, bWidth - 2, bHeight - 2, bWidth - 1, bHeight - 2, 0, bHeight - 2, bWidth - 2, bHeight - 1, 0, bHeight - 1, bWidth - 2, 0, bWidth - 1, 0, 0, 0);
	//flush
	for(var i = 0; i != bWidth; ++i){
		for(var j = 0; j != bHeight; ++j){
...
function cRun(){
	...
	running = !running;
	if(running){
		cmdRun.value = "Running"
		genHandle = setInterval("generation();", 0);
	}else{
